"""Worker / Beat 进程：根 logger 挂载 ERROR 滚动文件（与 API 进程逻辑一致，避免 worker 依赖 api 包）。"""
...
def attach_error_log_file(path: str, *, max_bytes: int = 10_485_760, backup_count: int = 5) -> None:
    ...
    if os.environ.get("IQUANT_ERROR_LOG_DEBUG", "").lower() in ("1", "true", "yes"):
        logging.getLogger(__name__).info("error_log_file_attached", extra={"path": resolved})
