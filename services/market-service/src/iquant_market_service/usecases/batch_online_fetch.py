...
async def _resolve_codes(
    *,
    ...
    """
    from iquant_market_data.tdx.a_share_list import list_a_share_full_codes
    from iquant_market_data.tdx.client import TdxClient
    ...
    def _fetch(client: TdxClient) -> list[str]:
        return list_a_share_full_codes(client)

    all_fc = await pool.run_sync(_fetch)
    if not all_fc:
        logger.error(
...
async def execute_batch_online_task(
    *,
    ...
    full_codes = await _resolve_codes(markets=markets, codes=codes_raw, pool=pool)
